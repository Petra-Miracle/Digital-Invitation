import { useTranslation } from "react-i18next";
import { Mail } from "lucide-react";

const ArrowIcon = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 20 20"
    fill="currentColor"
    className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-current group-hover:translate-x-0.5 transition-all duration-200"
  >
    <path
      fillRule="evenodd"
      d="M3 10a.75.75 0 01.75-.75h10.638L10.23 5.29a.75.75 0 111.04-1.08l5.5 5.25a.75.75 0 010 1.08l-5.5 5.25a.75.75 0 11-1.04-1.08l4.158-3.96H3.75A.75.75 0 013 10z"
      clipRule="evenodd"
    />
  </svg>
);

export default function KontakSection() {
  const { t } = useTranslation();

  const contacts = [
    {
      id: "whatsapp",
      label: "WhatsApp",
      description: t("kontak.whatsappDesc"),
      href: "https://wa.me/081353030694",
      gradient: "from-green-500 to-green-600",
      shadow: "shadow-green-900/20",
      border: "border-green-200 dark:border-green-900/40",
      hoverBorder: "hover:border-green-400 dark:hover:border-green-600",
      bg: "bg-green-50 dark:bg-green-950/20",
      hoverBg: "hover:bg-green-100/60 dark:hover:bg-green-950/40",
      textHover: "group-hover:text-green-700 dark:group-hover:text-green-400",
      icon: (
        <svg viewBox="0 0 24 24" fill="white" className="w-6 h-6">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
        </svg>
      ),
    },
    {
      id: "email",
      label: "Email",
      description: t("kontak.emailDesc"),
      href: "mailto:email:rivaldy.adoe@gmail.com",
      gradient: "from-blue-500 to-blue-700",
      shadow: "shadow-blue-900/20",
      border: "border-blue-200 dark:border-blue-900/40",
      hoverBorder: "hover:border-blue-400 dark:hover:border-blue-600",
      bg: "bg-blue-50 dark:bg-blue-950/20",
      hoverBg: "hover:bg-blue-100/60 dark:hover:bg-blue-950/40",
      textHover: "group-hover:text-blue-700 dark:group-hover:text-blue-400",
      icon: <Mail className="w-6 h-6 text-white" />,
    },
    {
      id: "instagram",
      label: "Instagram",
      description: t("kontak.instagramDesc"),
      href: "https://www.instagram.com/rivaldyadoe_?igsh=MWQ4anIzMnp2YXdlZw%3D%3D&utm_source=qr",
      gradient: "from-pink-500 via-rose-500 to-violet-600",
      shadow: "shadow-pink-900/20",
      border: "border-pink-200 dark:border-pink-900/40",
      hoverBorder: "hover:border-pink-400 dark:hover:border-pink-600",
      bg: "bg-pink-50 dark:bg-pink-950/20",
      hoverBg: "hover:bg-pink-100/60 dark:hover:bg-pink-950/40",
      textHover: "group-hover:text-pink-700 dark:group-hover:text-pink-400",
      icon: (
        <svg viewBox="0 0 24 24" fill="white" className="w-6 h-6">
          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
        </svg>
      ),
    },
  ];

  return (
    <section id="kontak" className="bg-slate-50 dark:bg-slate-950 py-20 px-4 sm:px-6 lg:px-8">
      {/* Decorative glow */}
      <div className="absolute inset-x-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-blue-100/40 dark:bg-blue-900/10 rounded-full blur-3xl" />
      </div>

      <div className="relative max-w-5xl mx-auto">

        {/* Section Header */}
        <div className="text-center mb-14">
          <span className="inline-flex items-center gap-3 text-xs font-bold tracking-widest text-blue-600 dark:text-blue-400 uppercase mb-4">
            <span className="w-8 h-0.5 bg-blue-500 rounded-full" />
            {t("kontak.sectionBadge")}
            <span className="w-8 h-0.5 bg-blue-500 rounded-full" />
          </span>
          <h2 className="text-4xl font-extrabold text-slate-900 dark:text-white sm:text-5xl">
            {t("kontak.title")}{" "}
            <span className="relative inline-block">
              <span className="text-blue-600 dark:text-blue-400">{t("kontak.titleHighlight")}</span>
              <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-gradient-to-r from-blue-500 to-blue-800 rounded-full" />
            </span>
          </h2>
          <p className="mt-4 text-slate-500 dark:text-slate-400 max-w-xl mx-auto">
            {t("kontak.subtitle")}
          </p>
        </div>

        {/* Contact Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {contacts.map((c) => (
            <a
              key={c.id}
              href={c.href}
              target="_blank"
              rel="noopener noreferrer"
              className={`group relative flex flex-col gap-5 p-6 rounded-2xl border transition-all duration-200 hover:-translate-y-1.5 ${c.border} ${c.hoverBorder} ${c.bg} ${c.hoverBg} shadow-sm`}
            >
              {/* Icon */}
              <div
                className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${c.gradient} flex items-center justify-center shadow-lg ${c.shadow} shrink-0`}
              >
                {c.icon}
              </div>

              {/* Text */}
              <div className="flex-1">
                <p className={`text-base font-bold text-slate-900 dark:text-white transition-colors ${c.textHover}`}>
                  {c.label}
                </p>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-300 mt-0.5 break-all">
                </p>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 leading-relaxed">
                  {c.description}
                </p>
              </div>

              {/* Arrow */}
              <div className="flex items-center justify-end">
                <ArrowIcon />
              </div>
            </a>
          ))}
        </div>

        {/* Bottom note */}
        <p className="text-center text-xs text-slate-400 dark:text-slate-600 mt-10">
          {t("kontak.bottomNote")}{" "}
          <span className="text-blue-500 font-semibold">{t("kontak.responseTime")}</span>
          {t("kontak.bottomEnd")}
        </p>

      </div>
    </section>
  );
}
